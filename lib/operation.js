    ...
    case "object":
      ...
      if (op === null) {
        ...
        switch (op.type) {
          ...
          case "replaceDOM":
            op = operation.replaceDOM(op.selector, op.output);
            break;
          ...
          case "file":
            op = operation.insert.file(op.file);
            break;
...
operation.replaceDOM = function(selector, output) {
  var manipulation = "replace";

  function replaceDOM(body) {
    if (!body || !body.length) return body;
    return _replaceDOM(body, selector);
  }

  function _replaceDOM(body, selector) {
    var $ = cheerio.load(body);
    var resolved = path.resolve(process.cwd(), output);
    var _content = fs.readFileSync(resolved).toString();

    if(manipulation === "append") {
      $(selector).append(_content);
    } else {
  ...
  replaceDOM.manipulation = function(_) {
    if (!arguments.length) return manipulation;
    manipulation = _;
    return replaceDOM;
  };
