  ...
  function inject(req, res, next) {
    ...
    return request[method].call(null,
      ...
      function(error, response, body) {
        ...
        var contentEncoding = (response.headers.hasOwnProperty('content-encoding')) ? response.headers['content-encoding'] : null;

        // handle gzip
        if (contentEncoding === 'gzip' || contentEncoding === 'deflate') {

          res.set('Content-Encoding', 'utf8');
        ...
        function processResponse(changed) {
          //body = bodyTemp;

          if (isInjectable(response)) {
            if (!changed) body = body.toString();
            ...
            if (changed) res.set("Content-length", Buffer.byteLength(body, 'utf8'));
            return res.send(body);
          }
        }
      });
